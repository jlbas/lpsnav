...
class Eval:
    ...
    def compute_legibility(self, env):
        ...
        other_agents = {
            id: agent for id, agent in env.agents.items() if agent != env.ego_agent
        }
        legibility_score = {id: 0 for id in other_agents}
        rel_prims = self.config.prim_horiz * np.multiply.outer(
            speeds, helper.unit_vec(self.rel_headings)
        )
        for id, agent in other_agents.items():
            ...
            for i in range(len(env.ego_agent.pos_log)):
                ...
                if in_front and in_radius:
                    ...
                    if not cost_tg_log:
                        cost_tg_log = np.full(
                            (int(self.config.receding_horiz / env.timestep), 3), cost_tg
                        )
                    cost_pg = helper.dynamic_prim_cost(
                        env.ego_agent.pos_log[i],
                        abs_prims,
                        env.ego_agent.max_speed,
                        abs_prims_vels,
                    ...
                    goal_conditional = (
                        np.exp(cost_sg - (cost_st + cost_tg)) * subgoal_priors
                    )
            ...
            discounted_t = np.linspace(
                len(goal_conditional_log) * self.config.timestep,
                self.config.timestep,
                len(goal_conditional_log),
            )
            legibility_score[id] = np.sum(
                np.max(goal_conditional_log, axis=-1) * discounted_t
            ) / np.sum(discounted_t)
