...
def snapshot(ego_agent, id):
    ...
    plt.pause(0.1)
    ax.scatter(ego_agent.pred_int_lines[id][:,0], ego_agent.pred_int_lines[id][:,1], color=ego_agent.other_agents[id].color)
    plt.pause(0.1)
...
class Animate():
    ...
    def ani(self, i, agents, last_frame, plt, fig):
        for a in agents:
            ...
            try:
                a.patches.body_poly.set_xy(helper.rotate(a.body_coords, a.heading_log[i]) + a.pos_log[i])
                a.patches.body.center = a.pos_log[i]
            except IndexError:
                pass

            if self.config.debug:
                if a.policy == 'legitable' and i < min([len(log)-1 for log in a.int_lines_log.values()]):
                    for log, circles in zip(a.int_lines_log.values(), a.patches.int_lines):
                        for pos, circle in zip(log[i], circles):
                            if pos is None or np.all(np.isnan(pos)):
                                ...
                                circle.center = pos
                    for log, circles in zip(a.pred_int_lines_log.values(), a.patches.pred_int_lines):
                        for pos, circle in zip(log[i], circles):
                            if pos is None or np.all(np.isnan(pos)):
                                circle.set_radius(0)
                            else:
                ...
                if a.policy == 'legitable':
                    for j, (prim, speed, col_row) in enumerate(zip(a.patches.prims, a.abs_prims_log[i], a.col_mask_log[i])):
    ...
    def init_ani(self, agents):
        ...
        for a in agents:
            ...
            a.patches.path = Polygon(((0,0), (0,0)), closed=False, fill=False, lw=5, zorder=0, color=a.color, capstyle='round')
            x = 0.9 * a.radius * np.cos(np.pi/6)
            y = 0.9 * a.radius * np.sin(np.pi/6)
            a.body_coords = [(0.9 * a.radius, 0), (-x, y), (-x, -y)]
            a.patches.body_poly = Polygon(a.body_coords, facecolor=ax.get_facecolor(), linewidth=4, zorder=3)
            a.patches.body = Circle((a.pos), a.radius, color=a.color, zorder=2, label=f'{type(a).__name__}')
            if self.config.debug:
                if a.policy == 'legitable':
                    a.patches.prims = ([[Circle((0,0), 0.04, color='#004D40', lw=0)
                        for _ in a.rel_headings] for _ in a.speeds])
                if a.policy == 'legitable':
                    a.patches.int_lines = [[Circle((0, 0), 0, color='gray'), Circle((0, 0), 0, color='gray')] for _ in a.int_lines_log]
                    a.patches.pred_int_lines = [[Circle((0, 0), 0, color=a.other_agents[id].color) for _ in a.pred_int_lines_log[id][0]] for id in a.pred_int_lines_log]
    ...
    def plot(self, agents):
        ...
        for i, a in enumerate(list(agents)[::-1]):
            ...
            if self.config.plot_traj:
                ...
                ax.plot(np.array(a.pos_log)[:,0], np.array(a.pos_log)[:,1], c=a.color, lw=2, solid_capstyle='round', zorder=len(agents)+i)
            if self.config.plot_body:
                hls_color = colorsys.rgb_to_hls(*mc.to_rgb(a.color))
                lightness_range = np.linspace(hls_color[1] + 0.2 * (1-hls_color[1]), 1 - 0.2 * (1-hls_color[1]), sampled_len)
